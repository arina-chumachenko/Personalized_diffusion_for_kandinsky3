...
def save_model_card(
    ...
    pipeline: Kandinsky3Pipeline = None,
):
    img_str = ""
...
"""
    ...
    if isinstance(pipeline, Kandinsky3Pipeline):
        tags.extend(["kandinsky-3", "kandinsky-3-diffusers"])
    else:
...
def log_validation(
    ...
    if movq is not None:
        pipeline_args["movq"] = movq

    unet.to(dtype=torch.float16)
    lora_layers.to(dtype=torch.float16)

    # create pipeline (note: unet and movq are loaded again in float32)
    ...
    lora_layers.to(dtype=torch.float32)
...
def parse_args(input_args=None):
    ...
    # parser.add_argument("--rescale_betas_zero_snr", type=bool, default=True)
    parser.add_argument(
        "--allow_tf32",
        action="store_true",
        help=(
    ...
    def __getitem__(self, index):
        ...
        if self.encoder_hidden_states is not None:
            ...
            text_inputs = tokenize_prompt(
                self.tokenizer, self.instance_prompt, tokenizer_max_length=128
            )
        ...
        if self.class_data_root:
            ...
            if self.class_prompt_encoder_hidden_states is not None:
                ...
                class_text_inputs = tokenize_prompt(
                    self.tokenizer, self.class_prompt, tokenizer_max_length=128
                )
...
def model_has_movq(args):
    config_file_name = Path("movq", VQModel.config_name).as_posix()
...
def main(args):
    ...
    if args.tokenizer_name:
        tokenizer = T5Tokenizer.from_pretrained(args.tokenizer_name, revision=args.revision)
    ...
    def load_model_hook(models, input_dir):
        while len(models) > 0:
            ...
            if isinstance(model, type(unwrap_model(text_encoder))):
                ...
                load_model = Kandinsky3UNet.from_pretrained(input_dir, subfolder="unet")
    ...
    for name, layer in unet.named_modules():
        if name.endswith('attention'):
            if 'block' in name:
                ...
                in_channels = layer.inner_dim
                context_dim = layer.cross_attention_dim

                lora_attn_procs[name + '.processor'] = LoraKandi3AttnProcessor(
                    in_channels=in_channels, out_channels=out_channels, context_dim=context_dim, rank=rank)
            else:
                lora_attn_procs[name + '.processor'] = lora_attn_procs[name + '.processor'] = LoraKandi3AttnProcessor(
                    in_channels=4096, out_channels=1536, context_dim=4096, rank=rank, scale=0.0)
    ...
    #     if 'block' in name and ('to_q' in name or 'to_k' in name or 'to_v' in name):
    #         param.requires_grad_(True)
            # if 'attentions.0.attention' not in name:
            #     print(name)
    ...
    # )
    # params_to_optimize = [p for p in unet.parameters() if p.requires_grad] + [p for p in text_encoder.parameters() if p.requires_grad]
    params_to_optimize = [p for p in lora_layers.parameters() if p.requires_grad]
    ...
    # For mixed precision training we cast all non-trainable weights (movq, non-lora text_encoder and non-lora unet) to half-precision
    ...
    # Move movq and text_encoder to device and cast to weight_dtype
    if movq is not None:
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if args.pre_compute_text_embeddings:
                    ...
                    # context_mask = batch.get("attention_mask", None)
                else:
                ...
                # # Ensure context_mask is not None before using it
                # if context_mask is not None:
                #     context_mask = context_mask.to(dtype=encoder_hidden_states.dtype)
